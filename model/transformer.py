...
class RADD(nn.Module, PyTorchModelHubMixin):
    def __init__(self, config):
        ...
        self.config = config
        self.vocab_size = config.tokens + 1 # N, mask index is N - 1
        self.length = config.model.length # D
        ...
        self.output_layer = DDitFinalLayerWot(config.model.hidden_size, self.vocab_size)
        if config.model.dtype == 'float64':
            self.dtype = torch.float64
        elif config.model.dtype == 'float32':
            ...
            raise ValueError(f"Invalid dtype {config.model.dtype}")

    def forward(self, indices):
        """
        input: [B, D]
        output: [B, D, N], [:, :, -1] is useless
        return: log(softmax(logits))
            so that after applying exp(), the last dimension 
            (except the last position) is a probability vector
        """
    ...
    def logits(self, indices):
        ...
        output: [B, D, N], [:, :, -1] is useless
        return: logits
            the last dimension (except the last position)
            needs to be softmaxed to become a probability vector
        """
        x = self.vocab_embed(indices)
        rotary_cos_sin = self.rotary_emb(x)
        with torch.amp.autocast('cuda',dtype=self.dtype):
